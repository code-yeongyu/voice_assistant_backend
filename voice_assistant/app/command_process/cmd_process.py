...
def _link_by_words(cmd, user) :
    ...
    if _is_contain(cmd, "버스") : #버스
        ...
        response = actions.BasicFeatures().meals(datetime.datetime.now().weekday(), user)
        return {'response':response, 'action':action}

    elif _is_contain(cmd, "티비") or _is_contain(cmd, "TV") or _is_contain(cmd, "tv"): #티비 컨트롤
        #볼륨 조절
        if _is_contain(cmd, "소리") or _is_contain(cmd, "볼륨") :
            #볼륨 높이기
            if _is_contain(cmd, "키워") or _is_contain(cmd, "높") or _is_contain(cmd, "올") :
                try :
                ...
                return {'response':response, 'action':action}
            #볼륨 낮추기
            elif _is_contain(cmd, "줄여") or _is_contain(cmd, "낮") or _is_contain(cmd, "내") :
                try :
                    level = int(re.findall('\d+', cmd)[0])
        ...
        #채널 조절
            ...
            #번호로 조작
            if _is_contain(cmd, "높") or _is_contain(cmd, "올") or _is_contain(cmd, "낮") or _is_contain(cmd, "내"): #단순 조작 모음
                #채널 up
                if _is_contain(cmd, "높") or _is_contain(cmd, "올") :
                    try :
                        level = int(re.findall('\d+', cmd)[0])
                        response = actions.TvSimpleControlRepeat().channel_control(True, level)
                    except :
                ...
                #채널 down
                    ...
                    return {'response':response, 'action':action}
            elif _is_contain(cmd, "번") or _is_contain(cmd, "틀어") or _is_contain(cmd, "돌려") or _is_contain(cmd, "바꿔") :
                try :
    ...
    elif _is_contain(cmd, "몇시") or _is_contain(cmd, "몇 시") or _is_contain(cmd, "시간"): #시간
        response = actions.BasicFeatures().time_now()
        return {'response':response, 'action':action}

    elif _is_contain(cmd, "몇일") or _is_contain(cmd, "며칠") or _is_contain(cmd, "몇 일") or _is_contain(cmd, "날짜"): #날짜
